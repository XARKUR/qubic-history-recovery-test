# Proposal to Update the Qbond Smart Contract

Allow the Qbond smart contract code to be updated in the 190th epoch:
...
## Overview of the Qbond Mechanism

The Qbond smart contract tokenizes staking positions. Each epoch issues MBNDXXX tokens (XXX is the epoch number), representing staking positions. A maximum of 53 MBND tokens exist in the Qubic network because Qearn rewards are distributed 53 weeks after issuance, after which the token is burned.

The core idea is to use staking‑position tokenization to improve user experience, simplify trading, and enable a DeFi ecosystem built around MBND assets. MBND is considered one of the most stable assets since it provides predictable returns by the end of its cycle, making it ideal for collateralization.

---

## Issue

The current smart contract model, which issues new tokens every week, would quickly deplete the contract’s fund reserves, significantly reducing the contract’s period of operation.

---

## Proposed Changes

### **1. Modify Token Issuance**
Instead of issuing new tokens weekly, the contract will:
- Issue **53 tokens (MBND01–MBND53)** at the epoch defined by:
  `constexpr uint16 QBOND_CYCLIC_START_EPOCH = 190`
- Supply for each token:
  `constexpr sint64 QBOND_MBONDS_EMISSION = 1000000000LL`
- Tokens will **not be burned** but reused every 53 epochs.
...
```
It stores which MBND index (1–53) corresponds to the current epoch.

The contract file must be expanded by 8 bytes:
```
dd if=/dev/zero bs=8 count=1 >> contract0017.190
```

### **3. Changes to `BEGIN_EPOCH_WITH_LOCALS`**
- Skip the contract’s own address during reward distribution after 53 epochs.
- If funds were locked before `QBOND_CYCLIC_START_EPOCH`, MBND tokens are burned; otherwise they return to the contract.
- Implement cyclic bond reuse: `_cyclicMbondCounter` increments each epoch and wraps from 53 back to 1.
- In each epoch, users receive MBNDXX based on the cycle (MBND01, MBND02, ..., MBND53 → then back to MBND01).

### **4. Remove Bond Burning in `END_EPOCH_WITH_LOCALS`**
Unused MBND tokens are no longer burned.

---

## Summary of Key Changes
- One‑time issuance of **53 MBND tokens**, reused every 53 epochs.
- MBND tokens are **returned to the contract** and reused instead of being burned.
...
<details>
<summary><strong>Show/hide updated code</strong></summary>
